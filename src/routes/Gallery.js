...
class Gallery extends React.Component {
    ...
    openGallery = (item) => {
        ...
        // const {pswpElement} = this.item.pswpElement;
        const pswpElement = document.querySelectorAll('.pswp')[0];
        const options = {index: 0};
        this.gallery = new PhotoSwipe( pswpElement, PhotoswipeUIDefault, items, options);
        const _this = this;
        this.gallery.listen('gettingData', (_index, item1) => {
            if (item1.w < 1 || item1.h < 1) { // unknown size
                const img = new Image();
                img.onload = () => { // will get size after load
                    item1.w = this.width; // set image width
                    item1.h = this.height; // set image height
                    _this.gallery.invalidateCurrItems(); // reinit Items
                    _this.gallery.updateSize(true); // reinit Items
                };
                img.src = item1.src; // let's download image
            }
        });
        this.gallery.init();
    };
    ...
    render() {
        ...
        const imgsTag = imgs.map(v1 => (
            v1.map(v2 => (
              <div className="gutter-box" key={v2} style={{padding:'10px',background:'#ECECEC',margin:'5px'}}>
                <Card bordered={false} bodyStyle={{ padding: 0 }}>
                  <Card.Grid style={{width:'100%',textAlign:'center'}}>
                    ...
                    <div className="pa-m">
                      <h4 style={{textAlign:'center'}}>img Title</h4>
                      <small><div style={{textAlign:'center'}}><a href="https://BetaLi.github.io/" target="_blank" rel="noopener noreferrer">https://BetaLi.github.io/</a></div></small>
                    </div>
                  </Card.Grid>
                </Card>
              </div>
        ...
        return (
          <div className="gutter-example button-demo">
            <Row gutter={10}>
              ...
              <Col className="gutter-row" md={5}>{imgsTag[1]}</Col>
              <Col className="gutter-row" md={5}>{imgsTag[2]}</Col>
              <Col className="gutter-row" md={5}>{imgsTag[3]}</Col>
              <Col className="gutter-row" md={4}>{imgsTag[4]}</Col>
            </Row>
              ...
              <div className="pswp__scroll-wrap">
                ...
                <div className="pswp__ui pswp__ui--hidden">
                  <div className="pswp__top-bar">
                    ...
                    <button className="pswp__button pswp__button--fs" title="Toggle fullscreen" type="button" />
                  ...
                  <button className="pswp__button pswp__button--arrow--left" title="Previous (arrow left)" type="button" />
                  <button className="pswp__button pswp__button--arrow--right" title="Next (arrow right)" type="button" />
